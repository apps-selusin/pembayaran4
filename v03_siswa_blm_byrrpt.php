...
//		$item->Visible = TRUE;
	...
	function GetExtendedFilter() {
		...
		// Reset search command
			...
			$this->SetSessionDropDownValue($this->Periode_Tahun_Bulan->DropDownValue, $this->Periode_Tahun_Bulan->SearchOperator, 'Periode_Tahun_Bulan'); // Field Periode_Tahun_Bulan
			...
			if ($this->GetDropDownValue($this->Periode_Tahun_Bulan)) {
				$bSetupFilter = TRUE;
			} elseif ($this->Periode_Tahun_Bulan->DropDownValue <> EWR_INIT_VALUE && !isset($_SESSION['sv_v03_siswa_blm_byr_Periode_Tahun_Bulan'])) {
				$bSetupFilter = TRUE;
			}
			...
			if ($this->GetDropDownValue($this->Periode_Text)) {
				$bSetupFilter = TRUE;
			} elseif ($this->Periode_Text->DropDownValue <> EWR_INIT_VALUE && !isset($_SESSION['sv_v03_siswa_blm_byr_Periode_Text'])) {
				$bSetupFilter = TRUE;
			}
		...
		if ($bRestoreSession) {
			...
			$this->GetSessionDropDownValue($this->Periode_Tahun_Bulan); // Field Periode_Tahun_Bulan
		...
		$this->BuildDropDownFilter($this->Periode_Tahun_Bulan, $sFilter, $this->Periode_Tahun_Bulan->SearchOperator, FALSE, TRUE); // Field Periode_Tahun_Bulan
		...
		// Field Periode_Tahun_Bulan
		ewr_LoadDropDownList($this->Periode_Tahun_Bulan->DropDownList, $this->Periode_Tahun_Bulan->DropDownValue);
	...
	function LoadDefaultFilters() {
		...
		*/
		...
		// Field Periode_Tahun_Bulan
		$this->Periode_Tahun_Bulan->DefaultDropDownValue = EWR_INIT_VALUE;
		if (!$this->SearchCommand) $this->Periode_Tahun_Bulan->DropDownValue = $this->Periode_Tahun_Bulan->DefaultDropDownValue;
	...
	function CheckFilter() {
		...
		// Check Periode_Tahun_Bulan extended filter
		if ($this->NonTextFilterApplied($this->Periode_Tahun_Bulan))
			return TRUE;
	...
	function ShowFilterList($showDate = FALSE) {
		...
		$sExtWrk = "";
		$sWrk = "";
		$this->BuildDropDownFilter($this->Periode_Tahun_Bulan, $sExtWrk, $this->Periode_Tahun_Bulan->SearchOperator);
		$sFilter = "";
		if ($sExtWrk <> "")
			$sFilter .= "<span class=\"ewFilterValue\">$sExtWrk</span>";
		elseif ($sWrk <> "")
			$sFilter .= "<span class=\"ewFilterValue\">$sWrk</span>";
		if ($sFilter <> "")
			$sFilterList .= "<div><span class=\"ewFilterCaption\">" . $this->Periode_Tahun_Bulan->FldCaption() . "</span>" . $sFilter . "</div>";
	...
	function GetFilterList() {
		...
		// Field Periode_Tahun_Bulan
		$sWrk = "";
		$sWrk = ($this->Periode_Tahun_Bulan->DropDownValue <> EWR_INIT_VALUE) ? $this->Periode_Tahun_Bulan->DropDownValue : "";
		if (is_array($sWrk))
			$sWrk = implode("||", $sWrk);
		if ($sWrk <> "")
			$sWrk = "\"sv_Periode_Tahun_Bulan\":\"" . ewr_JsEncode2($sWrk) . "\"";
		if ($sWrk <> "") {
			if ($sFilterList <> "") $sFilterList .= ",";
			$sFilterList .= $sWrk;
		}
	...
	function SetupFilterList($filter) {
		...
		// Field Periode_Tahun_Bulan
		$bRestoreFilter = FALSE;
		if (array_key_exists("sv_Periode_Tahun_Bulan", $filter)) {
			$sWrk = $filter["sv_Periode_Tahun_Bulan"];
			if (strpos($sWrk, "||") !== FALSE)
				$sWrk = explode("||", $sWrk);
			$this->SetSessionDropDownValue($sWrk, @$filter["so_Periode_Tahun_Bulan"], "Periode_Tahun_Bulan");
			$bRestoreFilter = TRUE;
		}
		if (!$bRestoreFilter) { // Clear filter
			$this->SetSessionDropDownValue(EWR_INIT_VALUE, "", "Periode_Tahun_Bulan");
		}
...
fv03_siswa_blm_byrrpt.Lists["sv_Periode_Tahun_Bulan"] = {"LinkField":"sv_Periode_Tahun_Bulan","Ajax":true,"DisplayFields":["sv_Periode_Tahun_Bulan","","",""],"ParentFields":[],"FilterFields":[],"Options":[],"Template":""};
fv03_siswa_blm_byrrpt.Lists["sv_Periode_Text"] = {"LinkField":"sv_Periode_Text","Ajax":true,"DisplayFields":["sv_Periode_Text","","",""],"ParentFields":[],"FilterFields":[],"Options":[],"Template":""};
</script>
<?php } ?>
<?php if ($Page->Export == "" && !$Page->DrillDown) { ?>
<script type="text/javascript">
...
<div id="c_Periode_Tahun_Bulan" class="ewCell form-group">
	<label for="sv_Periode_Tahun_Bulan" class="ewSearchCaption ewLabel"><?php echo $Page->Periode_Tahun_Bulan->FldCaption() ?></label>
	<span class="ewSearchField">
<?php ewr_PrependClass($Page->Periode_Tahun_Bulan->EditAttrs["class"], "form-control"); ?>
<select data-table="v03_siswa_blm_byr" data-field="x_Periode_Tahun_Bulan" data-value-separator="<?php echo ewr_HtmlEncode(is_array($Page->Periode_Tahun_Bulan->DisplayValueSeparator) ? json_encode($Page->Periode_Tahun_Bulan->DisplayValueSeparator) : $Page->Periode_Tahun_Bulan->DisplayValueSeparator) ?>" id="sv_Periode_Tahun_Bulan" name="sv_Periode_Tahun_Bulan"<?php echo $Page->Periode_Tahun_Bulan->EditAttributes() ?>>
<option value=""><?php echo $ReportLanguage->Phrase("PleaseSelect") ?></option>
<?php
	$cntf = is_array($Page->Periode_Tahun_Bulan->AdvancedFilters) ? count($Page->Periode_Tahun_Bulan->AdvancedFilters) : 0;
	$cntd = is_array($Page->Periode_Tahun_Bulan->DropDownList) ? count($Page->Periode_Tahun_Bulan->DropDownList) : 0;
	$totcnt = $cntf + $cntd;
	$wrkcnt = 0;
	if ($cntf > 0) {
		foreach ($Page->Periode_Tahun_Bulan->AdvancedFilters as $filter) {
			if ($filter->Enabled) {
				$selwrk = ewr_MatchedFilterValue($Page->Periode_Tahun_Bulan->DropDownValue, $filter->ID) ? " selected" : "";
?>
<option value="<?php echo $filter->ID ?>"<?php echo $selwrk ?>><?php echo $filter->Name ?></option>
<?php
				$wrkcnt += 1;
	...
	for ($i = 0; $i < $cntd; $i++) {
		$selwrk = " selected";
?>
<option value="<?php echo $Page->Periode_Tahun_Bulan->DropDownList[$i] ?>"<?php echo $selwrk ?>><?php echo ewr_DropDownDisplayValue($Page->Periode_Tahun_Bulan->DropDownList[$i], "", 0) ?></option>
<?php
		$wrkcnt += 1;
	}
?>
</select>
<input type="hidden" name="s_sv_Periode_Tahun_Bulan" id="s_sv_Periode_Tahun_Bulan" value="<?php echo $Page->Periode_Tahun_Bulan->LookupFilterQuery() ?>"></span>
</div>
</div>
