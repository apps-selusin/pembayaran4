...
$t03_kelas_list = NULL; // Initialize page object first

class ct03_kelas_list extends ct03_kelas {
	...
	// Page object name
	var $PageObjName = 't03_kelas_list';

	// Grid form hidden field names
	var $FormName = 'ft03_kelaslist';
	...
	function __construct() {
		...
		if (!isset($GLOBALS["t03_kelas"]) || get_class($GLOBALS["t03_kelas"]) == "ct03_kelas") {
			$GLOBALS["t03_kelas"] = &$this;
		...
		if (!defined("EW_TABLE_NAME"))
			define("EW_TABLE_NAME", 't03_kelas', TRUE);
		...
		$this->FilterOptions->TagClassName = "ewFilterOption ft03_kelaslistsrch";
	...
	function Page_Terminate($url = "") {
		...
		// Export
		global $EW_EXPORT, $t03_kelas;
		if ($this->CustomExport <> "" && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, $EW_EXPORT)) {
				$sContent = ob_get_contents();
			if ($gsExportFile == "") $gsExportFile = $this->TableVar;
			$class = $EW_EXPORT[$this->CustomExport];
			if (class_exists($class)) {
				$doc = new $class($t03_kelas);
				$doc->Text = $sContent;
	...
	function GetFilterList() {
		...
		if (EW_SEARCH_FILTER_OPTION == "Server") {
			$sSavedFilterList = isset($UserProfile) ? $UserProfile->GetSearchFilters(CurrentUserName(), "ft03_kelaslistsrch") : "";
		} else {
			$sSavedFilterList = "";
		}
	...
	function ProcessFilterList() {
		...
		if (@$_POST["ajax"] == "savefilters") { // Save filter request (Ajax)
			...
			$UserProfile->SetSearchFilters(CurrentUserName(), "ft03_kelaslistsrch", $filters);
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"ft03_kelaslistsrch\" href=\"#\">" . $Language->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"ft03_kelaslistsrch\" href=\"#\">" . $Language->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
		...
		if ($this->CurrentAction <> "gridadd" && $this->CurrentAction <> "gridedit") { // Not grid add/edit mode
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"ewAction ewListAction\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.ft03_kelaslist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
...
// Create page object
if (!isset($t03_kelas_list)) $t03_kelas_list = new ct03_kelas_list();

// Page init
$t03_kelas_list->Page_Init();

// Page main
$t03_kelas_list->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$t03_kelas_list->Page_Render();
?>
...
ft03_kelaslist.ValidateRequired = true;
<?php } else { ?>
ft03_kelaslist.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
ft03_kelaslist.Lists["x_sekolah_id"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_Sekolah","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"t02_sekolah"};
...
<?php if ($t03_kelas_list->TotalRecs > 0 && $t03_kelas_list->ExportOptions->Visible()) { ?>
<?php $t03_kelas_list->ExportOptions->Render("body") ?>
<?php } ?>
<?php if ($t03_kelas_list->SearchOptions->Visible()) { ?>
<?php $t03_kelas_list->SearchOptions->Render("body") ?>
<?php } ?>
<?php if ($t03_kelas_list->FilterOptions->Visible()) { ?>
<?php $t03_kelas_list->FilterOptions->Render("body") ?>
<?php } ?>
...
if ($t03_kelas->CurrentAction == "gridadd") {
	...
	if ($bSelectLimit) {
		if ($t03_kelas_list->TotalRecs <= 0)
			$t03_kelas_list->TotalRecs = $t03_kelas->SelectRecordCount();
	} else {
		if (!$t03_kelas_list->Recordset && ($t03_kelas_list->Recordset = $t03_kelas_list->LoadRecordset()))
			$t03_kelas_list->TotalRecs = $t03_kelas_list->Recordset->RecordCount();
	}
	$t03_kelas_list->StartRec = 1;
	if ($t03_kelas_list->DisplayRecs <= 0 || ($t03_kelas->Export <> "" && $t03_kelas->ExportAll)) // Display all records
		$t03_kelas_list->DisplayRecs = $t03_kelas_list->TotalRecs;
	if (!($t03_kelas->Export <> "" && $t03_kelas->ExportAll))
		$t03_kelas_list->SetUpStartRec(); // Set up start record position
	if ($bSelectLimit)
		$t03_kelas_list->Recordset = $t03_kelas_list->LoadRecordset($t03_kelas_list->StartRec-1, $t03_kelas_list->DisplayRecs);

	// Set no record found message
	if ($t03_kelas->CurrentAction == "" && $t03_kelas_list->TotalRecs == 0) {
		if ($t03_kelas_list->SearchWhere == "0=101")
			$t03_kelas_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
...
<?php } ?>
<?php $t03_kelas_list->ShowPageHeader(); ?>
<?php
$t03_kelas_list->ShowMessage();
?>
<?php if ($t03_kelas_list->TotalRecs > 0 || $t03_kelas->CurrentAction <> "") { ?>
<div class="panel panel-default ewGrid t03_kelas">
<form name="ft03_kelaslist" id="ft03_kelaslist" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($t03_kelas_list->CheckToken) { ?>
...
// Header row
$t03_kelas_list->RowType = EW_ROWTYPE_HEADER;

// Render list options
$t03_kelas_list->RenderListOptions();

// Render list options (header, left)
$t03_kelas_list->ListOptions->Render("header", "left");
?>
<?php if ($t03_kelas->sekolah_id->Visible) { // sekolah_id ?>
	...
	<?php } else { ?>
		<th data-name="sekolah_id"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $t03_kelas->SortUrl($t03_kelas->sekolah_id) ?>',2);"><div id="elh_t03_kelas_sekolah_id" class="t03_kelas_sekolah_id">
...
$t03_kelas_list->ListOptions->Render("header", "right");
?>
	</tr>
</thead>
<tbody>
<?php
if ($t03_kelas->ExportAll && $t03_kelas->Export <> "") {
	$t03_kelas_list->StopRec = $t03_kelas_list->TotalRecs;
} else {

	// Set the last record to display
	if ($t03_kelas_list->TotalRecs > $t03_kelas_list->StartRec + $t03_kelas_list->DisplayRecs - 1)
		$t03_kelas_list->StopRec = $t03_kelas_list->StartRec + $t03_kelas_list->DisplayRecs - 1;
	else
		$t03_kelas_list->StopRec = $t03_kelas_list->TotalRecs;
}
...
if ($t03_kelas_list->Recordset && !$t03_kelas_list->Recordset->EOF) {
	...
	$bSelectLimit = $t03_kelas_list->UseSelectLimit;
	if (!$bSelectLimit && $t03_kelas_list->StartRec > 1)
		$t03_kelas_list->Recordset->Move($t03_kelas_list->StartRec - 1);
} elseif (!$t03_kelas->AllowAddDeleteRow && $t03_kelas_list->StopRec == 0) {
	$t03_kelas_list->StopRec = $t03_kelas->GridAddRowCount;
}
...
$t03_kelas_list->ListOptions->Render("body", "left", $t03_kelas_list->RowCnt);
?>
	<?php if ($t03_kelas->sekolah_id->Visible) { // sekolah_id ?>
...
// Close recordset
if ($t03_kelas_list->Recordset)
	$t03_kelas_list->Recordset->Close();
?>
<div class="panel-footer ewGridLowerPanel">
<?php if ($t03_kelas->CurrentAction <> "gridadd" && $t03_kelas->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="ewForm form-inline ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($t03_kelas_list->Pager)) $t03_kelas_list->Pager = new cPrevNextPager($t03_kelas_list->StartRec, $t03_kelas_list->DisplayRecs, $t03_kelas_list->TotalRecs) ?>
<?php if ($t03_kelas_list->Pager->RecordCount > 0 && $t03_kelas_list->Pager->Visible) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($t03_kelas_list->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $t03_kelas_list->PageUrl() ?>start=<?php echo $t03_kelas_list->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($t03_kelas_list->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $t03_kelas_list->PageUrl() ?>start=<?php echo $t03_kelas_list->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $t03_kelas_list->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($t03_kelas_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $t03_kelas_list->PageUrl() ?>start=<?php echo $t03_kelas_list->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($t03_kelas_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $t03_kelas_list->PageUrl() ?>start=<?php echo $t03_kelas_list->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $t03_kelas_list->Pager->PageCount ?></span>
</div>
<div class="ewPager ewRec">
	<span><?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $t03_kelas_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $t03_kelas_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $t03_kelas_list->Pager->RecordCount ?></span>
</div>
<?php } ?>
</form>
<?php } ?>
...
<?php
	foreach ($t03_kelas_list->OtherOptions as &$option)
...
<?php } ?>
<?php if ($t03_kelas_list->TotalRecs == 0 && $t03_kelas->CurrentAction == "") { // Show other options ?>
<div class="ewListOtherOptions">
<?php
	foreach ($t03_kelas_list->OtherOptions as &$option) {
		$option->ButtonClass = "";
...
<?php } ?>
<script type="text/javascript">
ft03_kelaslistsrch.FilterList = <?php echo $t03_kelas_list->GetFilterList() ?>;
ft03_kelaslistsrch.Init();
ft03_kelaslist.Init();
</script>
<?php
$t03_kelas_list->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
