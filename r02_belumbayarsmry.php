...
<?php include_once "r02_belumbayarsmryinfo.php" ?>
<?php
...
$r02_belumbayar_summary = NULL; // Initialize page object first

class crr02_belumbayar_summary extends crr02_belumbayar {
	...
	// Page object name
	var $PageObjName = 'r02_belumbayar_summary';
	...
	function __construct() {
		...
		// Table object (r02_belumbayar)
		if (!isset($GLOBALS["r02_belumbayar"])) {
			$GLOBALS["r02_belumbayar"] = &$this;
			$GLOBALS["Table"] = &$GLOBALS["r02_belumbayar"];
		}
		...
		if (!defined("EWR_TABLE_NAME"))
			define("EWR_TABLE_NAME", 'r02_belumbayar', TRUE);
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fr02_belumbayarsummary";
	...
	function Page_Init() {
		...
		// Setup placeholder
		$this->Periode_Text->PlaceHolder = $this->Periode_Text->FldCaption();
...
//		$item->Visible = TRUE;
		...
		$item->Body = "<a title=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" data-caption=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" id=\"emf_r02_belumbayar\" href=\"javascript:void(0);\" onclick=\"ewr_EmailDialogShow({lnk:'emf_r02_belumbayar',hdr:ewLanguage.Phrase('ExportToEmail'),url:'$url',exportid:'$exportid',el:this});\">" . $ReportLanguage->Phrase("ExportToEmail") . "</a>";
		$item->Visible = FALSE;
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fr02_belumbayarsummary\" href=\"#\">" . $ReportLanguage->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fr02_belumbayarsummary\" href=\"#\">" . $ReportLanguage->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-caption=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-toggle=\"button\" data-form=\"fr02_belumbayarsummary\">" . $ReportLanguage->Phrase("SearchBtn") . "</button>";
		$item->Visible = TRUE;
	...
	function GetExtendedFilter() {
		...
		// Reset search command
			...
			$this->SetSessionFilterValues($this->Periode_Text->SearchValue, $this->Periode_Text->SearchOperator, $this->Periode_Text->SearchCondition, $this->Periode_Text->SearchValue2, $this->Periode_Text->SearchOperator2, 'Periode_Text'); // Field Periode_Text
			...
			if ($this->GetDropDownValue($this->NIS)) {
				$bSetupFilter = TRUE;
			} elseif ($this->NIS->DropDownValue <> EWR_INIT_VALUE && !isset($_SESSION['sv_r02_belumbayar_NIS'])) {
				$bSetupFilter = TRUE;
			}
			...
			if ($this->GetDropDownValue($this->Nama)) {
				$bSetupFilter = TRUE;
			} elseif ($this->Nama->DropDownValue <> EWR_INIT_VALUE && !isset($_SESSION['sv_r02_belumbayar_Nama'])) {
				$bSetupFilter = TRUE;
			}
			...
			if ($this->GetDropDownValue($this->Kelas)) {
				$bSetupFilter = TRUE;
			} elseif ($this->Kelas->DropDownValue <> EWR_INIT_VALUE && !isset($_SESSION['sv_r02_belumbayar_Kelas'])) {
				$bSetupFilter = TRUE;
			}
			...
			if ($this->GetDropDownValue($this->Sekolah)) {
				$bSetupFilter = TRUE;
			} elseif ($this->Sekolah->DropDownValue <> EWR_INIT_VALUE && !isset($_SESSION['sv_r02_belumbayar_Sekolah'])) {
				$bSetupFilter = TRUE;
			}

			// Field Periode_Text
			if ($this->GetFilterValues($this->Periode_Text)) {
				$bSetupFilter = TRUE;
			}
		...
		if ($bRestoreSession) {
			...
			$this->GetSessionFilterValues($this->Periode_Text); // Field Periode_Text
		}
		...
		$this->BuildExtendedFilter($this->Periode_Text, $sFilter, FALSE, TRUE); // Field Periode_Text
	...
	function GetSessionDropDownValue(&$fld) {
		$parm = substr($fld->FldVar, 2);
		$this->GetSessionValue($fld->DropDownValue, 'sv_r02_belumbayar_' . $parm);
		$this->GetSessionValue($fld->SearchOperator, 'so_r02_belumbayar_' . $parm);
	}

	// Get filter values from session
	function GetSessionFilterValues(&$fld) {
		$parm = substr($fld->FldVar, 2);
		$this->GetSessionValue($fld->SearchValue, 'sv_r02_belumbayar_' . $parm);
		$this->GetSessionValue($fld->SearchOperator, 'so_r02_belumbayar_' . $parm);
		$this->GetSessionValue($fld->SearchCondition, 'sc_r02_belumbayar_' . $parm);
		$this->GetSessionValue($fld->SearchValue2, 'sv2_r02_belumbayar_' . $parm);
		$this->GetSessionValue($fld->SearchOperator2, 'so2_r02_belumbayar_' . $parm);
	}
	...
	function SetSessionDropDownValue($sv, $so, $parm) {
		$_SESSION['sv_r02_belumbayar_' . $parm] = $sv;
		$_SESSION['so_r02_belumbayar_' . $parm] = $so;
	}

	// Set filter values to session
	function SetSessionFilterValues($sv1, $so1, $sc, $sv2, $so2, $parm) {
		$_SESSION['sv_r02_belumbayar_' . $parm] = $sv1;
		$_SESSION['so_r02_belumbayar_' . $parm] = $so1;
		$_SESSION['sc_r02_belumbayar_' . $parm] = $sc;
		$_SESSION['sv2_r02_belumbayar_' . $parm] = $sv2;
		$_SESSION['so2_r02_belumbayar_' . $parm] = $so2;
	}
	...
	function ClearSessionSelection($parm) {
		$_SESSION["sel_r02_belumbayar_$parm"] = "";
		$_SESSION["rf_r02_belumbayar_$parm"] = "";
		$_SESSION["rt_r02_belumbayar_$parm"] = "";
	}

	// Load selection from session
	function LoadSelectionFromSession($parm) {
		$fld = &$this->FieldByParm($parm);
		$fld->SelectionList = @$_SESSION["sel_r02_belumbayar_$parm"];
		$fld->RangeFrom = @$_SESSION["rf_r02_belumbayar_$parm"];
		$fld->RangeTo = @$_SESSION["rt_r02_belumbayar_$parm"];
	}
	...
	function LoadDefaultFilters() {
		...
		*/
		...
		// Field Periode_Text
		$this->SetDefaultExtFilter($this->Periode_Text, "LIKE", NULL, 'AND', "=", NULL);
		if (!$this->SearchCommand) $this->ApplyDefaultExtFilter($this->Periode_Text);
		/**
	...
	function CheckFilter() {
		...
		// Check Periode_Text text filter
		if ($this->TextFilterApplied($this->Periode_Text))
			return TRUE;
		return FALSE;
	}
	...
	function ShowFilterList($showDate = FALSE) {
		...
		$sExtWrk = "";
		$sWrk = "";
		$this->BuildExtendedFilter($this->Periode_Text, $sExtWrk);
		$sFilter = "";
		if ($sExtWrk <> "")
			$sFilter .= "<span class=\"ewFilterValue\">$sExtWrk</span>";
		elseif ($sWrk <> "")
	...
	function GetFilterList() {
		...
		// Field Periode_Text
		$sWrk = "";
		if ($this->Periode_Text->SearchValue <> "" || $this->Periode_Text->SearchValue2 <> "") {
			$sWrk = "\"sv_Periode_Text\":\"" . ewr_JsEncode2($this->Periode_Text->SearchValue) . "\"," .
				"\"so_Periode_Text\":\"" . ewr_JsEncode2($this->Periode_Text->SearchOperator) . "\"," .
				"\"sc_Periode_Text\":\"" . ewr_JsEncode2($this->Periode_Text->SearchCondition) . "\"," .
				"\"sv2_Periode_Text\":\"" . ewr_JsEncode2($this->Periode_Text->SearchValue2) . "\"," .
				"\"so2_Periode_Text\":\"" . ewr_JsEncode2($this->Periode_Text->SearchOperator2) . "\"";
		}
		if ($sWrk <> "") {
			if ($sFilterList <> "") $sFilterList .= ",";
			$sFilterList .= $sWrk;
	...
	function SetupFilterList($filter) {
		...
		// Field Periode_Text
		$bRestoreFilter = FALSE;
		if (array_key_exists("sv_Periode_Text", $filter) || array_key_exists("so_Periode_Text", $filter) ||
			array_key_exists("sc_Periode_Text", $filter) ||
			array_key_exists("sv2_Periode_Text", $filter) || array_key_exists("so2_Periode_Text", $filter)) {
			$this->SetSessionFilterValues(@$filter["sv_Periode_Text"], @$filter["so_Periode_Text"], @$filter["sc_Periode_Text"], @$filter["sv2_Periode_Text"], @$filter["so2_Periode_Text"], "Periode_Text");
			$bRestoreFilter = TRUE;
		}
		if (!$bRestoreFilter) { // Clear filter
			$this->SetSessionFilterValues("", "=", "AND", "", "=", "Periode_Text");
		}
		return TRUE;
	}
...
// Create page object
if (!isset($r02_belumbayar_summary)) $r02_belumbayar_summary = new crr02_belumbayar_summary();
if (isset($Page)) $OldPage = $Page;
$Page = &$r02_belumbayar_summary;
...
// Create page object
var r02_belumbayar_summary = new ewr_Page("r02_belumbayar_summary");

// Page properties
r02_belumbayar_summary.PageID = "summary"; // Page ID
var EWR_PAGE_ID = r02_belumbayar_summary.PageID;

// Extend page with Chart_Rendering function
r02_belumbayar_summary.Chart_Rendering = 
...
r02_belumbayar_summary.Chart_Rendered = 
 function(chart, chartid) { // DO NOT CHANGE THIS LINE!
...
// Form object
var CurrentForm = fr02_belumbayarsummary = new ewr_Form("fr02_belumbayarsummary");

// Validate method
fr02_belumbayarsummary.Validate = function() {
...
fr02_belumbayarsummary.Form_CustomValidate = 
...
fr02_belumbayarsummary.ValidateRequired = true; // Uses JavaScript validation
<?php } else { ?>
fr02_belumbayarsummary.ValidateRequired = false; // No JavaScript validation
<?php } ?>

// Use Ajax
fr02_belumbayarsummary.Lists["sv_NIS"] = {"LinkField":"sv_NIS","Ajax":true,"DisplayFields":["sv_NIS","","",""],"ParentFields":[],"FilterFields":[],"Options":[],"Template":""};
fr02_belumbayarsummary.Lists["sv_Nama"] = {"LinkField":"sv_Nama","Ajax":true,"DisplayFields":["sv_Nama","","",""],"ParentFields":[],"FilterFields":[],"Options":[],"Template":""};
fr02_belumbayarsummary.Lists["sv_Kelas"] = {"LinkField":"sv_Kelas","Ajax":true,"DisplayFields":["sv_Kelas","","",""],"ParentFields":[],"FilterFields":[],"Options":[],"Template":""};
fr02_belumbayarsummary.Lists["sv_Sekolah"] = {"LinkField":"sv_Sekolah","Ajax":true,"DisplayFields":["sv_Sekolah","","",""],"ParentFields":[],"FilterFields":[],"Options":[],"Template":""};
</script>
<?php } ?>
<?php if ($Page->Export == "" && !$Page->DrillDown) { ?>
<script type="text/javascript">
...
<?php if ($Page->Export == "" && !$Page->DrillDown) { ?>
<!-- Search form (begin) -->
<form name="fr02_belumbayarsummary" id="fr02_belumbayarsummary" class="form-inline ewForm ewExtFilterForm" action="<?php echo ewr_CurrentPage() ?>">
<?php $SearchPanelClass = ($Page->Filter <> "") ? " in" : " in"; ?>
<div id="fr02_belumbayarsummary_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
...
<select data-table="r02_belumbayar" data-field="x_NIS" data-value-separator="<?php echo ewr_HtmlEncode(is_array($Page->NIS->DisplayValueSeparator) ? json_encode($Page->NIS->DisplayValueSeparator) : $Page->NIS->DisplayValueSeparator) ?>" id="sv_NIS" name="sv_NIS"<?php echo $Page->NIS->EditAttributes() ?>>
...
<select data-table="r02_belumbayar" data-field="x_Nama" data-value-separator="<?php echo ewr_HtmlEncode(is_array($Page->Nama->DisplayValueSeparator) ? json_encode($Page->Nama->DisplayValueSeparator) : $Page->Nama->DisplayValueSeparator) ?>" id="sv_Nama" name="sv_Nama"<?php echo $Page->Nama->EditAttributes() ?>>
<option value=""><?php echo $ReportLanguage->Phrase("PleaseSelect") ?></option>
<?php
...
<select data-table="r02_belumbayar" data-field="x_Kelas" data-value-separator="<?php echo ewr_HtmlEncode(is_array($Page->Kelas->DisplayValueSeparator) ? json_encode($Page->Kelas->DisplayValueSeparator) : $Page->Kelas->DisplayValueSeparator) ?>" id="sv_Kelas" name="sv_Kelas"<?php echo $Page->Kelas->EditAttributes() ?>>
<option value=""><?php echo $ReportLanguage->Phrase("PleaseSelect") ?></option>
<?php
...
<select data-table="r02_belumbayar" data-field="x_Sekolah" data-value-separator="<?php echo ewr_HtmlEncode(is_array($Page->Sekolah->DisplayValueSeparator) ? json_encode($Page->Sekolah->DisplayValueSeparator) : $Page->Sekolah->DisplayValueSeparator) ?>" id="sv_Sekolah" name="sv_Sekolah"<?php echo $Page->Sekolah->EditAttributes() ?>>
<option value=""><?php echo $ReportLanguage->Phrase("PleaseSelect") ?></option>
<?php
...
<div id="c_Periode_Text" class="ewCell form-group">
	...
	<span class="ewSearchOperator"><?php echo $ReportLanguage->Phrase("LIKE"); ?><input type="hidden" name="so_Periode_Text" id="so_Periode_Text" value="LIKE"></span>
	<span class="control-group ewSearchField">
<?php ewr_PrependClass($Page->Periode_Text->EditAttrs["class"], "form-control"); // PR8 ?>
<input type="text" data-table="r02_belumbayar" data-field="x_Periode_Text" id="sv_Periode_Text" name="sv_Periode_Text" size="30" maxlength="14" placeholder="<?php echo $Page->Periode_Text->PlaceHolder ?>" value="<?php echo ewr_HtmlEncode($Page->Periode_Text->SearchValue) ?>"<?php echo $Page->Periode_Text->EditAttributes() ?>>
</span>
</div>
</div>
...
<script type="text/javascript">
fr02_belumbayarsummary.Init();
fr02_belumbayarsummary.FilterList = <?php echo $Page->GetFilterList() ?>;
</script>
<!-- Search form (end) -->
<?php } ?>
...
<?php include "r02_belumbayarsmrypager.php" ?>
<div class="clearfix"></div>
</div>
<?php } ?>
<?php if ($Page->Export <> "pdf") { ?>
...
<?php } ?>
<span data-class="tpb<?php echo $Page->GrpCount-1 ?>_r02_belumbayar"><?php echo $Page->PageBreakContent ?></span>
<?php } ?>
<?php if ($Page->Export <> "pdf") { ?>
...
<?php if ($Page->Sekolah->Visible) { ?>
	...
	<?php } else { ?>
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="Sekolah"><div class="r02_belumbayar_Sekolah"><span class="ewTableHeaderCaption"><?php echo $Page->Sekolah->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="Sekolah">
<?php if ($Page->SortUrl($Page->Sekolah) == "") { ?>
		<div class="ewTableHeaderBtn r02_belumbayar_Sekolah">
			<span class="ewTableHeaderCaption"><?php echo $Page->Sekolah->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer r02_belumbayar_Sekolah" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->Sekolah) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->Sekolah->FldCaption() ?></span>
...
<?php if ($Page->Kelas->Visible) { ?>
	...
	<?php } else { ?>
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="Kelas"><div class="r02_belumbayar_Kelas"><span class="ewTableHeaderCaption"><?php echo $Page->Kelas->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="Kelas">
<?php if ($Page->SortUrl($Page->Kelas) == "") { ?>
		<div class="ewTableHeaderBtn r02_belumbayar_Kelas">
			<span class="ewTableHeaderCaption"><?php echo $Page->Kelas->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer r02_belumbayar_Kelas" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->Kelas) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->Kelas->FldCaption() ?></span>
...
<?php if ($Page->NIS->Visible) { ?>
	...
	<?php } else { ?>
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="NIS"><div class="r02_belumbayar_NIS"><span class="ewTableHeaderCaption"><?php echo $Page->NIS->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="NIS">
<?php if ($Page->SortUrl($Page->NIS) == "") { ?>
		<div class="ewTableHeaderBtn r02_belumbayar_NIS">
			<span class="ewTableHeaderCaption"><?php echo $Page->NIS->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer r02_belumbayar_NIS" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->NIS) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->NIS->FldCaption() ?></span>
...
<?php if ($Page->Nama->Visible) { ?>
	...
	<?php } else { ?>
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="Nama"><div class="r02_belumbayar_Nama"><span class="ewTableHeaderCaption"><?php echo $Page->Nama->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="Nama">
<?php if ($Page->SortUrl($Page->Nama) == "") { ?>
		<div class="ewTableHeaderBtn r02_belumbayar_Nama">
			<span class="ewTableHeaderCaption"><?php echo $Page->Nama->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer r02_belumbayar_Nama" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->Nama) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->Nama->FldCaption() ?></span>
...
<?php if ($Page->Jenis->Visible) { ?>
	...
	<?php } else { ?>
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="Jenis"><div class="r02_belumbayar_Jenis"><span class="ewTableHeaderCaption"><?php echo $Page->Jenis->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="Jenis">
<?php if ($Page->SortUrl($Page->Jenis) == "") { ?>
		<div class="ewTableHeaderBtn r02_belumbayar_Jenis">
			<span class="ewTableHeaderCaption"><?php echo $Page->Jenis->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer r02_belumbayar_Jenis" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->Jenis) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->Jenis->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="Periode_Tahun_Bulan"><div class="r02_belumbayar_Periode_Tahun_Bulan"><span class="ewTableHeaderCaption"><?php echo $Page->Periode_Tahun_Bulan->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="Periode_Tahun_Bulan">
<?php if ($Page->SortUrl($Page->Periode_Tahun_Bulan) == "") { ?>
		<div class="ewTableHeaderBtn r02_belumbayar_Periode_Tahun_Bulan">
			<span class="ewTableHeaderCaption"><?php echo $Page->Periode_Tahun_Bulan->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer r02_belumbayar_Periode_Tahun_Bulan" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->Periode_Tahun_Bulan) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->Periode_Tahun_Bulan->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="Periode_Text"><div class="r02_belumbayar_Periode_Text"><span class="ewTableHeaderCaption"><?php echo $Page->Periode_Text->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="Periode_Text">
<?php if ($Page->SortUrl($Page->Periode_Text) == "") { ?>
		<div class="ewTableHeaderBtn r02_belumbayar_Periode_Text">
			<span class="ewTableHeaderCaption"><?php echo $Page->Periode_Text->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer r02_belumbayar_Periode_Text" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->Periode_Text) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->Periode_Text->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="Nilai"><div class="r02_belumbayar_Nilai"><span class="ewTableHeaderCaption"><?php echo $Page->Nilai->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="Nilai">
<?php if ($Page->SortUrl($Page->Nilai) == "") { ?>
		<div class="ewTableHeaderBtn r02_belumbayar_Nilai">
			<span class="ewTableHeaderCaption"><?php echo $Page->Nilai->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer r02_belumbayar_Nilai" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->Nilai) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->Nilai->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
		<span class="ewSummaryCaption r02_belumbayar_Sekolah"><span class="ewTableHeaderCaption"><?php echo $Page->Sekolah->FldCaption() ?></span></span>
<?php } else { ?>
	<?php if ($Page->SortUrl($Page->Sekolah) == "") { ?>
		<span class="ewSummaryCaption r02_belumbayar_Sekolah">
			<span class="ewTableHeaderCaption"><?php echo $Page->Sekolah->FldCaption() ?></span>
		</span>
	<?php } else { ?>
		<span class="ewTableHeaderBtn ewPointer ewSummaryCaption r02_belumbayar_Sekolah" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->Sekolah) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->Sekolah->FldCaption() ?></span>
			<span class="ewTableHeaderSort"><?php if ($Page->Sekolah->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($Page->Sekolah->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span>
		</span>
	<?php } ?>
...
<span data-class="tpx<?php echo $Page->GrpCount ?>_r02_belumbayar_Sekolah"<?php echo $Page->Sekolah->ViewAttributes() ?>><?php echo $Page->Sekolah->GroupViewValue ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
		<span class="ewSummaryCaption r02_belumbayar_Kelas"><span class="ewTableHeaderCaption"><?php echo $Page->Kelas->FldCaption() ?></span></span>
<?php } else { ?>
	<?php if ($Page->SortUrl($Page->Kelas) == "") { ?>
		<span class="ewSummaryCaption r02_belumbayar_Kelas">
			<span class="ewTableHeaderCaption"><?php echo $Page->Kelas->FldCaption() ?></span>
		</span>
	<?php } else { ?>
		<span class="ewTableHeaderBtn ewPointer ewSummaryCaption r02_belumbayar_Kelas" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->Kelas) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->Kelas->FldCaption() ?></span>
			<span class="ewTableHeaderSort"><?php if ($Page->Kelas->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($Page->Kelas->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span>
		</span>
	<?php } ?>
<?php } ?>
		<?php echo $ReportLanguage->Phrase("SummaryColon") ?>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->GrpCounter[0] ?>_r02_belumbayar_Kelas"<?php echo $Page->Kelas->ViewAttributes() ?>><?php echo $Page->Kelas->GroupViewValue ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
		<span class="ewSummaryCaption r02_belumbayar_NIS"><span class="ewTableHeaderCaption"><?php echo $Page->NIS->FldCaption() ?></span></span>
<?php } else { ?>
	<?php if ($Page->SortUrl($Page->NIS) == "") { ?>
		<span class="ewSummaryCaption r02_belumbayar_NIS">
			<span class="ewTableHeaderCaption"><?php echo $Page->NIS->FldCaption() ?></span>
		</span>
	<?php } else { ?>
		<span class="ewTableHeaderBtn ewPointer ewSummaryCaption r02_belumbayar_NIS" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->NIS) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->NIS->FldCaption() ?></span>
			<span class="ewTableHeaderSort"><?php if ($Page->NIS->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($Page->NIS->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span>
		</span>
	<?php } ?>
<?php } ?>
		<?php echo $ReportLanguage->Phrase("SummaryColon") ?>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->GrpCounter[0] ?>_<?php echo $Page->GrpCounter[1] ?>_r02_belumbayar_NIS"<?php echo $Page->NIS->ViewAttributes() ?>><?php echo $Page->NIS->GroupViewValue ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
		<span class="ewSummaryCaption r02_belumbayar_Nama"><span class="ewTableHeaderCaption"><?php echo $Page->Nama->FldCaption() ?></span></span>
<?php } else { ?>
	<?php if ($Page->SortUrl($Page->Nama) == "") { ?>
		<span class="ewSummaryCaption r02_belumbayar_Nama">
			<span class="ewTableHeaderCaption"><?php echo $Page->Nama->FldCaption() ?></span>
		</span>
	<?php } else { ?>
		<span class="ewTableHeaderBtn ewPointer ewSummaryCaption r02_belumbayar_Nama" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->Nama) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->Nama->FldCaption() ?></span>
			<span class="ewTableHeaderSort"><?php if ($Page->Nama->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($Page->Nama->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span>
		</span>
	<?php } ?>
<?php } ?>
		<?php echo $ReportLanguage->Phrase("SummaryColon") ?>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->GrpCounter[0] ?>_<?php echo $Page->GrpCounter[1] ?>_<?php echo $Page->GrpCounter[2] ?>_r02_belumbayar_Nama"<?php echo $Page->Nama->ViewAttributes() ?>><?php echo $Page->Nama->GroupViewValue ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
		<span class="ewSummaryCaption r02_belumbayar_Jenis"><span class="ewTableHeaderCaption"><?php echo $Page->Jenis->FldCaption() ?></span></span>
<?php } else { ?>
	<?php if ($Page->SortUrl($Page->Jenis) == "") { ?>
		<span class="ewSummaryCaption r02_belumbayar_Jenis">
			<span class="ewTableHeaderCaption"><?php echo $Page->Jenis->FldCaption() ?></span>
		</span>
	<?php } else { ?>
		<span class="ewTableHeaderBtn ewPointer ewSummaryCaption r02_belumbayar_Jenis" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->Jenis) ?>',2);">
			<span class="ewTableHeaderCaption"><?php echo $Page->Jenis->FldCaption() ?></span>
			<span class="ewTableHeaderSort"><?php if ($Page->Jenis->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($Page->Jenis->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span>
		</span>
	<?php } ?>
<?php } ?>
		<?php echo $ReportLanguage->Phrase("SummaryColon") ?>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->GrpCounter[0] ?>_<?php echo $Page->GrpCounter[1] ?>_<?php echo $Page->GrpCounter[2] ?>_<?php echo $Page->GrpCounter[3] ?>_r02_belumbayar_Jenis"<?php echo $Page->Jenis->ViewAttributes() ?>><?php echo $Page->Jenis->GroupViewValue ?></span>
...
<span data-class="tpx<?php echo $Page->GrpCount ?>_r02_belumbayar_Sekolah"<?php echo $Page->Sekolah->ViewAttributes() ?>><?php echo $Page->Sekolah->GroupViewValue ?></span></td>
	<?php } ?>
<?php } ?>
<?php if ($Page->Kelas->Visible) { ?>
	<?php if ($Page->Kelas->ShowGroupHeaderAsRow) { ?>
...
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->GrpCounter[0] ?>_r02_belumbayar_Kelas"<?php echo $Page->Kelas->ViewAttributes() ?>><?php echo $Page->Kelas->GroupViewValue ?></span></td>
	<?php } ?>
<?php } ?>
<?php if ($Page->NIS->Visible) { ?>
	<?php if ($Page->NIS->ShowGroupHeaderAsRow) { ?>
...
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->GrpCounter[0] ?>_<?php echo $Page->GrpCounter[1] ?>_r02_belumbayar_NIS"<?php echo $Page->NIS->ViewAttributes() ?>><?php echo $Page->NIS->GroupViewValue ?></span></td>
	<?php } ?>
<?php } ?>
<?php if ($Page->Nama->Visible) { ?>
	<?php if ($Page->Nama->ShowGroupHeaderAsRow) { ?>
...
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->GrpCounter[0] ?>_<?php echo $Page->GrpCounter[1] ?>_<?php echo $Page->GrpCounter[2] ?>_r02_belumbayar_Nama"<?php echo $Page->Nama->ViewAttributes() ?>><?php echo $Page->Nama->GroupViewValue ?></span></td>
	<?php } ?>
<?php } ?>
<?php if ($Page->Jenis->Visible) { ?>
	<?php if ($Page->Jenis->ShowGroupHeaderAsRow) { ?>
...
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->GrpCounter[0] ?>_<?php echo $Page->GrpCounter[1] ?>_<?php echo $Page->GrpCounter[2] ?>_<?php echo $Page->GrpCounter[3] ?>_r02_belumbayar_Jenis"<?php echo $Page->Jenis->ViewAttributes() ?>><?php echo $Page->Jenis->GroupViewValue ?></span></td>
	<?php } ?>
<?php } ?>
<?php if ($Page->Periode_Tahun_Bulan->Visible) { ?>
		<td data-field="Periode_Tahun_Bulan"<?php echo $Page->Periode_Tahun_Bulan->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->GrpCounter[0] ?>_<?php echo $Page->GrpCounter[1] ?>_<?php echo $Page->GrpCounter[2] ?>_<?php echo $Page->GrpCounter[3] ?>_<?php echo $Page->RecCount ?>_r02_belumbayar_Periode_Tahun_Bulan"<?php echo $Page->Periode_Tahun_Bulan->ViewAttributes() ?>><?php echo $Page->Periode_Tahun_Bulan->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->Periode_Text->Visible) { ?>
		<td data-field="Periode_Text"<?php echo $Page->Periode_Text->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->GrpCounter[0] ?>_<?php echo $Page->GrpCounter[1] ?>_<?php echo $Page->GrpCounter[2] ?>_<?php echo $Page->GrpCounter[3] ?>_<?php echo $Page->RecCount ?>_r02_belumbayar_Periode_Text"<?php echo $Page->Periode_Text->ViewAttributes() ?>><?php echo $Page->Periode_Text->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->Nilai->Visible) { ?>
		<td data-field="Nilai"<?php echo $Page->Nilai->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->GrpCount ?>_<?php echo $Page->GrpCounter[0] ?>_<?php echo $Page->GrpCounter[1] ?>_<?php echo $Page->GrpCounter[2] ?>_<?php echo $Page->GrpCounter[3] ?>_<?php echo $Page->RecCount ?>_r02_belumbayar_Nilai"<?php echo $Page->Nilai->ViewAttributes() ?>><?php echo $Page->Nilai->ListViewValue() ?></span></td>
<?php } ?>
	</tr>
<?php
...
<?php if ($Page->Nilai->Visible) { ?>
		<td data-field="Nilai"<?php echo $Page->Nilai->CellAttributes() ?>>
<span data-class="tpgs<?php echo $Page->GrpCount ?>_<?php echo $Page->GrpCounter[0] ?>_<?php echo $Page->GrpCounter[1] ?>_r02_belumbayar_Nilai"<?php echo $Page->Nilai->ViewAttributes() ?>><?php echo $Page->Nilai->SumViewValue ?></span></td>
<?php } ?>
	</tr>
<?php } ?>
<?php
...
<?php } ?>
<?php if ($Page->Nilai->Visible) { ?>
		<td data-field="Nilai"<?php echo $Page->Nilai->CellAttributes() ?>><?php echo $ReportLanguage->Phrase("RptSum") ?>=<span data-class="tpts_r02_belumbayar_Nilai"<?php echo $Page->Nilai->ViewAttributes() ?>><?php echo $Page->Nilai->SumViewValue ?></span></td>
<?php } ?>
	</tr>
<?php } else { ?>
...
<?php if ($Page->Nilai->Visible) { ?>
		<td data-field="Nilai"<?php echo $Page->Nilai->CellAttributes() ?>>
<span data-class="tpts_r02_belumbayar_Nilai"<?php echo $Page->Nilai->ViewAttributes() ?>><?php echo $Page->Nilai->SumViewValue ?></span></td>
<?php } ?>
	</tr>
<?php } ?>
	</tfoot>
